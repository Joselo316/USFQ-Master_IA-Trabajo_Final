"""
Script para inferir una imagen individual con el modelo FastFlow (Modelo 4).
...
def inferir_imagen(
    ...
    """
    Infiere una imagen individual con FastFlow.
    
    Args:
        imagen_path: Ruta a la imagen
        modelo_path: Ruta al modelo entrenado (.pt)
        backbone: Backbone usado ('resnet18' o 'resnet50')
    ...
    if not modelo_path.is_absolute():
        ...
        posibles_rutas = [
            ...
            Path.cwd() / 'modelos' / 'modelo4_fastflow' / 'models' / modelo_path.name,  # Desde raíz del proyecto
        ]
    ...
    # Determinar el tamaño de entrada según el modo
    input_size = patch_size if usar_parches else img_size
    
    # Crear modelo con la misma configuración que el entrenamiento
    # Nota: Si el checkpoint tiene configuración guardada, úsala
    model = FastFlow(
    ...
    with open(output_path_txt, 'w', encoding='utf-8') as f:
        f.write(f"Resultado de Inferencia - FastFlow\n")
        f.write(f"{'='*50}\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description='Inferencia individual con FastFlow (Modelo 4)'
    )
    parser.add_argument(
        '--imagen',
        type=str,
