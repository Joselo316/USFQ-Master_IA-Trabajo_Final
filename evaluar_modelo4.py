"""
Script para evaluar el modelo 4 (FastFlow) usando imágenes etiquetadas.
...
MODELOS_DIR = PROJECT_ROOT / "modelos" / "modelo4_fastflow" / "models"
OUTPUT_DIR = PROJECT_ROOT / "evaluaciones" / "modelo4"
...
def crear_dataset_validacion(etiquetadas_dir: Path, img_size: int = 256) -> Tuple[DataLoader, List[int]]:
    ...
    Las imágenes ya están preprocesadas (3 canales), solo se cargan y normalizan.
    """
    # Obtener imágenes y etiquetas
    imagenes, etiquetas = obtener_imagenes_etiquetadas(etiquetadas_dir)
    
    if len(imagenes) == 0:
        raise ValueError(f"No se encontraron imágenes en {etiquetadas_dir}")
...
def evaluar_modelo(
    ...
    print(f"\n{'='*70}")
    print(f"EVALUANDO: FastFlow ({backbone})")
    print(f"{'='*70}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Evaluar modelo 4 (FastFlow) usando imágenes etiquetadas',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Este script evalúa el modelo 4 (FastFlow) usando imágenes procesadas de validación
...
Calcula métricas: accuracy, precision, recall, F1-score, specificity, confusion matrix, ROC curve.
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Directorio con modelos (default: modelos/modelo4_fastflow/models/ o models_256/)'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
        default=None,
        help='Directorio de salida (default: evaluaciones/modelo4/ o modelo4_256/)'
    )
    parser.add_argument(
        '--backbone',
        type=str,
    ...
    if args.output_dir:
        ...
        if args.redimensionar:
            output_dir = base_output_dir / "modelo4_256"
        else:
            output_dir = base_output_dir / "modelo4"
    ...
    # Buscar modelos
    modelo_pattern = f"fastflow_{args.backbone}_{args.img_size}.pt"
